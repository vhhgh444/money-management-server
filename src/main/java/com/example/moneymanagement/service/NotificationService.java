    ...
    public void sendDailyIncomeExpenseReminder(){
        ...
        for(ProfileEntity profile:profiles){
            String body="Hi"+profile.getFullName()+ ",<br><br>"
                    ...
                    +"<a href="+frontendUrl+"style='display:inline-block;padding:10px 20px;background-color:#4CAF50;color:#fff;text-decoration:none;border-radius:5px;font-weight:bold;font-weight:bold;'>Go to Money Manager</a>"
  ...
  // @Scheduled(cron = "0 * * * * *",zone ="IST")
    public void sendDailyExpenseSummary(){
        ...
        for(ProfileEntity profile:profiles){
            ...
            log.info("Profile: {}",profile.getEmail());
            log.info("Today's expense count: {}",todaysExpenses.size());
            if(!todaysExpenses.isEmpty()){
                log.info("Preparing email for {}",profile.getEmail());
